...
# config['learning_rate'] = 0.009  # Learning rate of gradient descent algorithm #used for sigmoid
# config['learning_rate'] = 0.003  # Learning rate of gradient descent algorithm #used for tanh
...
class Activation:
  ...
  def grad_ReLU(self):
    ...
    return np.greater(self.x,zeros).astype(int)
...
def trainer(model, X_train, y_train, X_valid, y_valid, X_test, y_test, config):
  ...
  plt.ylabel("% Accuracy")
  plt.title("ReLU: Accuracy vs. Epoch")
  plt.legend()
  plt.show()
